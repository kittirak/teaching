...
- **เปรียบเทียบผลลัพธ์ของ `ping` ทั้ง 2 ส่วน:**
  - ในส่วนของ **Local Network** Packet จะเดินทางจากเครื่องเราไปยัง Router และกลับมาอย่างรวดเร็ว
  - ในส่วนของ **Remote Network** Packet จะเดินทางผ่าน Router หลายตัว (Hop) เพื่อไปยังปลายทาง ทำให้ใช้เวลาเดินทางนานกว่า
- **วิเคราะห์ค่า TTL:**
  - **TTL** คือค่าที่ใช้ป้องกันไม่ให้ Packet วิ่งวนอยู่ในเครือข่ายอย่างไม่มีที่สิ้นสุด
  - เมื่อ Packet ผ่าน Router ไปแต่ละตัว ค่า TTL จะถูกลดลง 1
  - ดังนั้นการ `ping` ไปยัง **Remote Network** ที่อยู่ห่างไกลออกไป ค่า TTL ที่ตอบกลับมาจึงมีค่าน้อยกว่าค่าเริ่มต้น (ซึ่งปกติคือ 64 หรือ 128)
